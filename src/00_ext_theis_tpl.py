...
for i, step in enumerate(time):
    ...
    color = "C" + str(i + 1)
    plt.plot(rad, head_KG[i], label=label_TG, color=color, linestyle="--")
    plt.plot(rad, head_KH[i], label=label_TH, color=color, linestyle=":")
    plt.plot(rad, head_ef[i], label=label_ef, color=color)
    text_v = head_ef[i][-1]
    plt.annotate(
        time_labels[i],
        xy=(rad[-1], text_v),
...
plt.tight_layout()
plt.savefig(os.path.join("..", "results", "00_ext_theis_tpl_2D.png"), dpi=150)
plt.show()
